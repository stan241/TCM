-- Store 7: Controlled-Vocab Registry
-- TCM-ARCH-004 v4 §IV — Low sensitivity
-- Versioned result_type / result_subtype definitions.
-- v1.0.0 bootstrap. New entries require explicit registration + version bump.
-- MAJOR bump: new result_type. MINOR bump: new result_subtype.
-- Deprecated values accepted 90 days after bump, then rejected at normalizer.

CREATE TABLE vocab_version (
...
CREATE TABLE vocab_result_type (
  id              SERIAL      PRIMARY KEY,
  vocab_version   TEXT        NOT NULL REFERENCES vocab_version(version),
  result_type     TEXT        NOT NULL,
  description     TEXT        NOT NULL,
  deprecated      BOOLEAN     NOT NULL DEFAULT false,
  deprecated_at   TIMESTAMPTZ,
  -- Deprecated values accepted 90 days after deprecated_at, then rejected
  rejection_at    TIMESTAMPTZ GENERATED ALWAYS AS (deprecated_at + INTERVAL '90 days') STORED,
  UNIQUE(vocab_version, result_type)
);

CREATE TABLE vocab_result_subtype (
  id              SERIAL      PRIMARY KEY,
  vocab_version   TEXT        NOT NULL REFERENCES vocab_version(version),
  result_type     TEXT        NOT NULL,
...
-- v1.0.0 bootstrap seed
INSERT INTO vocab_version (version, released_by, notes) VALUES ('1.0.0', 'system', 'Initial controlled vocabulary');

INSERT INTO vocab_result_type (vocab_version, result_type, description) VALUES
...
INSERT INTO vocab_result_subtype (vocab_version, result_type, result_subtype, description) VALUES
